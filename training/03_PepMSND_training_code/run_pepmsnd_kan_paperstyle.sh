...
GPUS=(1)
EXP_ROOT="results_Pepmsnd_kan_models3"
LOG_ROOT="main/logs/launcher"
KAN_EXTRA_ARGS=""

# Set APPEND_META_TO_KAN=1 in environment to run KAN with descriptors+species/env (142-dim) input.
if [[ "${APPEND_META_TO_KAN:-0}" == "1" ]]; then
  KAN_EXTRA_ARGS="--append_meta_to_kan"
  EXP_ROOT="results_pepmsnd_kan_paperstyle_desc_plus_meta"
fi
...
# Build job queue (3 models x 10 folds)
...
while (( JOB_ID < TOTAL_JOBS )); do
  ...
  for gpu in "${FREE_GPUS[@]}"; do
    ...
    LOG_FILE="${LOG_ROOT}/pepmsnd_kan_paperstyle_${MODEL//\//_}_fold${FOLD}3.log"
    ...
    CUDA_VISIBLE_DEVICES=$gpu \
      .venv/bin/python main/scripts/train_pepmsnd_kan_paperstyle.py \
        --model_name "$MODEL" \
        --fold "$FOLD" \
        --save_path "$SAVE_PATH" \
        --seed 103 \
...
echo "========================================="
echo "All PepMSND paper-style KAN jobs complete."
echo "========================================="
